    ...
    where TFitness : IComparable<TFitness>
{
        ...
        int poolSize = 1, CrossoverDelegate crossover = null, int? maxSeconds = null)
    {
        ...
        var strategyLookup =
            ...
            {
                {Strategy.Create, (_, _, _) => FnGenerateParent()},
                {Strategy.Mutate, (parentGenes, _, _) => FnMutate(parentGenes)},
                {
        ...
        int numParents = 10, int maxGenerations = 100)
    {
        var pool = Enumerable.Range(0, 1 + numParents * numParents)
            .Select(_ => new Tuple<List<TGene>, int[]>(fnGenerateParent(), [0, 0, 0])).ToList();
        ...
        for (var generation = 0; generation < maxGenerations; generation++)
        {
            ...
            pool = (from i in Enumerable.Range(0, parents.Count)
                    ...
                    where i != j
                    select new Tuple<List<TGene>, int[]>(fnCrossover(parents[i], parents[j]), [0, 0, 0]))
                .ToList();
            pool.AddRange(parents.Select(parent => new Tuple<List<TGene>, int[]>(parent, [0, 0, 0])));
            pool.Add(new Tuple<List<TGene>, int[]>(fnGenerateParent(), [0, 0, 0]));
        }
