...
public class TicTacToeTests
{
    ...
    public static Fitness GetFitnessForAllGames(IList<Rule> genes)
    {
        ...
        // If the opponent starts, let them start at each square
        foreach (var square in board.Values)
        {
            var candidateCopy = new Dictionary<int, Square>(board)
            {
                [square.Index] = new(square.Index, ContentType.Opponent)
            };
            queue.Enqueue(candidateCopy);
        }
        ...
        while (queue.Any())
        {
            ...
            // queue all remaining OPPONENT responses
    ...
    public static bool MutateSwapAdjacent(List<Rule> genes)
    {
        if (genes.Count < 2)
            return false;
        var index = Rand.Random.Next(genes.Count - 1);
        (genes[index + 1], genes[index]) = (genes[index], genes[index + 1]);
        return true;
    }
        ...
        IReadOnlyList<FnMutateDelegate> mutationOperators, List<int> mutationRoundCounts)
    {
        ...
        {
            foreach (var mutate in mutationOperators.OrderBy(_ => Rand.Random.Next()))
            {
    ...
    public static Rule[] CreateGeneSet()
    {
        var options = new[]
        {
            new Tuple<ContentType?, int[]>(ContentType.Opponent, [0, 1, 2]),
            new Tuple<ContentType?, int[]>(ContentType.Mine, [0, 1, 2]),
        };
    ...
    public void TournamentTest()
    {
        ...
        void FnMutate(List<Rule> genes) =>
            Mutate(genes, _ => new Fitness(0, 0, 0, 0), mutationOperators, mutationRoundCounts);
