...
public class MagicSquareTests
{
    private static Fitness GetFitness(IReadOnlyList<int> genes, int diagonalSize, int expectedSum)
    {
        ...
        var sumOfDifferences = sums.Item1.Concat(sums.Item2).Concat([sums.Item3, sums.Item4])
    ...
    private static void Generate(int diagonalSize, int? maxAge)
    {
        ...
        List<int> FnCreate() => geneSet.OrderBy(_ => Rand.Random.Next(geneSet.Length)).ToList();
    ...
    {
        var historicalFitness = Enumerable.Range(0, 26).Select(n => new Fitness(50 - 2 * n)).ToArray();
        var index = Array.BinarySearch(historicalFitness, new Fitness(13));
    ...
    {
        var historicalFitness = Enumerable.Range(0, 26).Select(n => new Fitness(50 - 2 * n)).ToArray();
        var index = Array.BinarySearch(historicalFitness, new Fitness(12));
        if (index < 0) index = ~index;
        Assert.AreEqual(19, index);
    }
    ...
    {
        var historicalFitness = Enumerable.Range(1, count).Select(f => new Fitness(f)).Reverse().ToList();
        var index = historicalFitness.BinarySearch(value);
        if (index < 0) index = ~index;
    ...
    public void SimulatedAnnealingExpTest()
    {
        var samples = new[]
        {
            // index, difference, proportion singular, exp(-proportion)
    ...
    public void SearchFitnessTest()
    {
        ...
        var history = new List<Fitness> {new(6), new(4), new(2)};
        for (var fitness = 0; fitness < 8; fitness++)
        {
