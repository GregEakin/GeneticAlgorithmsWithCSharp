...
public class GraphColoringTests
{
    ...
    public void BuildRulesTest()
    {
        var items = new[]
        {
            new Tuple<string, string[]>("AK", []),
            new Tuple<string, string[]>("CA", ["TX", "FL"]),
            new Tuple<string, string[]>("TX", ["CA", "FL"]),
            new Tuple<string, string[]>("FL", ["CA", "TX"]),
        };
    ...
    {
        Color(@"Data\adjacent_states.col", ["Orange", "Yellow", "Green", "Blue"]);
    }

    [TestMethod]
    public void R100_1gbTest()
    {
        Color(@"Data\R100_1gb.col", ["Red", "Orange", "Yellow", "Green", "Blue", "Indigo"]);
    }
    ...
    private static void Color(string file, string[] colors)
    {
        ...
        var count = 0;
        var nodeIndexLookup = nodes.OrderBy(s => s).ToDictionary(k => k, _ => count++);
