    ...
    public void DiversityTest()
    {
        ...
        for (var i = 3; i <= initialInput.Count - 1; i++)
        {
                ...
                {
                    {
                        ...
                        eqResults.AddOrUpdate(result, 1, (_, count) => count + 1);
...

                    {
                        ...
                        var result = string.Join("", genesCopy);
                        geResults.AddOrUpdate(result, 1, (_, count) => count + 1);
            ...
            Console.WriteLine("{0} eq variations: {1}", eqResults.Count,
                string.Join(", ",
                    eqResults.OrderBy(x => x.Key).Select(x => $"{x.Key}({x.Value})")));
            Console.WriteLine("{0} ge variations: {1}", geResults.Count,
                string.Join(", ",
                    geResults.OrderBy(x => x.Key).Select(x => $"{x.Key}({x.Value})")));
        }
    }
}